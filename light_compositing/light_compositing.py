...
def read_images(directory, count):
    """
...
def object_modifier(directory, output, mask, fill=1, edge=1, diffuse=1,
                    count=-1, verbose=False):
    """
    This function calculates the per object modifier for an image.

    Arguments:
    directory -- the directory where the fill light, diffuse light, and
                 edge light images are located
...
def soft_modifier(directory, output, sigma, count=-1, verbose=False):
    ...
    """
    img_list = read_images(directory, count)
    modifier = modifier_lights.ModifierLights(img_list)
...
def regional_modifier(image_path, output, beta, verbose=False):
    ...
    modifier = modifier_lights.ModifierLights()
